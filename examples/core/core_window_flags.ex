...
--------------------------------------------------------------------------------------
    ...
    */
    ...
    SetConfigFlags(or_all({FLAG_VSYNC_HINT , FLAG_MSAA_4X_HINT , FLAG_WINDOW_HIGHDPI,FLAG_WINDOW_RESIZABLE}))
    ...
    sequence X1 = { 0, 0, GetScreenWidth(), GetScreenHeight() }
    atom ballRadius = 20
    ...
    do
        -- Update
        -------------------------------------------------------
        if (IsKeyPressed(KEY_F)) then ToggleFullscreen() end if  -- modifies window size when scaling!
        ...
        then
            ...
            else SetWindowState(FLAG_WINDOW_UNDECORATED) end if
        end if
        ...
        then
            framesCounter+=1
            if (framesCounter >= 240) then ClearWindowState(FLAG_WINDOW_HIDDEN)end if -- Show window after 3 seconds
        end if

        if (IsKeyPressed(KEY_N))
        then
            if not IsWindowState(FLAG_WINDOW_MINIMIZED) then  MinimizeWindow() end if
        ...
        then
            framesCounter+=1
            if (framesCounter >= 240) then RestoreWindow() end if -- Restore window after 3 seconds
        end if
        ...
        X1[width]=GetScreenWidth()
        X1[height]=GetScreenHeight()
        DrawRectangleLinesEx(X1,4, BLACK)
        ...
        else DrawText("FLAG_WINDOW_HIGHDPI: off", 10, 320, 10, MAROON) end if
        
        if (IsWindowState(FLAG_WINDOW_TRANSPARENT)) then DrawText("FLAG_WINDOW_TRANSPARENT: on", 10, 340, 10, LIME)
        else DrawText("FLAG_WINDOW_TRANSPARENT: off", 10, 340, 10, MAROON) end if
